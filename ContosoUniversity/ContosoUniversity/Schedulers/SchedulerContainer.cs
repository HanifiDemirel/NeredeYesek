    ...
    public class SchedulerContainer
    {
        public void RunJob()
        {
            try
            {
                ...
                ISimpleTrigger triggerMonthly = (ISimpleTrigger)TriggerBuilder.Create().WithIdentity("MonthlyScheduler").StartAt(DateTime.Now.AddSeconds(2)).WithSimpleSchedule(x => x.WithIntervalInSeconds(200).RepeatForever()).Build();
                ...
                ISimpleTrigger triggerDaily = (ISimpleTrigger)TriggerBuilder.Create().WithIdentity("DailyScheduler").StartAt(DateTime.Now.AddSeconds(5)).WithSimpleSchedule(x => x.WithIntervalInSeconds(3).RepeatForever()).Build();

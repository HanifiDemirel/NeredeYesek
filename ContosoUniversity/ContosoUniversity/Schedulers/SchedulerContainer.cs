    ...
    public class SchedulerContainer
    {
        public void RunJob()
        {
            try
            {
                ...
                ISimpleTrigger triggerMonthly = (ISimpleTrigger)TriggerBuilder.Create().WithIdentity("MonthlyScheduler").StartAt(DateTime.Now.AddSeconds(3)).WithSimpleSchedule(x => x.WithIntervalInSeconds(200).RepeatForever()).Build();
